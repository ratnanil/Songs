...
for (file in list.files("songs_md2",pattern = ".Rmd",full.names = TRUE)){
  file.remove(file)
}
...
for (dir in dirs){
  ...
  if(length(yml) > 0){
    ...
    for (song in songs){
      ...
      song_modified[chordpro_environment_l$tab] <- "```{class.source='chordgrid'}" # todo: only add thsi to the first instance
